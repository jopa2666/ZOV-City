...
local shit = {
    ...
    ["ent_armor_crate"] = "ent_armor_crate",
    ["ent_food_crate"] = "ent_food_crate",
    ["ent_pistol_crate"] = "ent_pistol_crate",
...
hook.Add("Think","Interact-Glow",function()
    ...
    if LatestShow > 0.05 then
        halo.Add({LatestEntity},Color(255,0,0,0 * LatestShow),1,1,5)
    else
        LatestEntity = NULL
    end
end)
...
/*hook.Add("HUDPaint","Interact-Shit",function()
    ...
    if IsValid(LatestEntity) then
        ...
        draw.SimpleText((printt != nil and printt or LatestEntity.PrintName),"HS.18",ScrW()/2,ScrH()/1.85 - 10 * (1 - LatestShow),Color(255,0,0,0 * LatestShow),TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
    end
end)