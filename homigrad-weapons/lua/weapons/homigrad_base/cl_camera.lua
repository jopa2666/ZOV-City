...
function SWEP:Camera(ply, origin, angles, fov)
	...
	govno_recoil = LerpFT(4,govno_recoil,0)

	lerpaim = LerpFT(self:IsSighted() and 0.05 or 0.10, lerpaim, self:IsSighted() and 1 or 0)

	if !no_fov:GetBool() then
		addfov = -(self.addfov or 5) * lerpaim
	end
	...
	if !self:IsSighted() then
		MAMA_KSILANA_SHLUHA = LerpAngleFT(0.6,MAMA_KSILANA_SHLUHA,angles)
	elseif self:IsSighted() then
		MAMA_KSILANA_SHLUHA = LerpAngleFT(0.45,MAMA_KSILANA_SHLUHA,ang)
	end
	...
	//ang = ang + Angle(76 * vis_recoil,0,0)
