...
function SWEP:PostShoot(Pos, Ang)
    ...
    if CLIENT then
        ...
        local Tr = {
            start = Pos,
            endpos = Pos + Ang:Forward() * 60000,
            filter = {hg.GetCurrentCharacter(self:GetOwner()), self}
        }
        local tr = util.TraceLine(Tr)

        for i = 1, 5 do
            local part = ParticleEmitter(Pos):Add("glide/effects/flamelet"..i, Pos)
            if part then
                part:SetDieTime(Rand(1.5 / 45, 1.5 / 35))
                part:SetColor(colorMuzzle.r, colorMuzzle.g, colorMuzzle.b)
                part:SetLighting(false)
                part:SetStartAlpha(Rand(65, 95))
                part:SetEndAlpha(Rand(0, 75))
                part:SetStartSize(Rand(4, 6) * 0.5)
                part:SetEndSize(Rand(5, 7))
        ...
        local part = ParticleEmitter(Pos):Add("glide/effects/skybox_boundary", Pos + Vector(Rand(-1,1), Rand(-1,1), Rand(-1,1)))
        if part then
            part:SetDieTime(0.065)
            part:SetColor(165, 95, 0)
            part:SetLighting(false)
            part:SetStartAlpha(20)
            part:SetEndAlpha(0)
            part:SetStartSize(Rand(4, 7))
            part:SetEndSize(random(35, 38))
            part:SetRoll(Rand(360, -360))
...
-- СКИДИБИ ДОП ДОП СПАСИТЕ ПОЖАЛУСТА Я ХОЧУ ДАМОЙ
-- Z DST<FK CFLCFKFNF Расшифруйте