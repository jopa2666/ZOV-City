...
if SERVER then
	function TOOL:GetConVars()
		return self:GetClientNumber( "fwd" ), self:GetClientNumber( "bck" ), self:GetClientNumber( "stop" ), self:GetClientNumber( "torque" ), self:GetClientNumber("scale")
	end

	function TOOL:MakeEnt( ply, model, Ang, trace,scale )
		scale = scale or 1
		...
		local fwd,bck,stop,torque,scale = self:GetConVars() -- These are the ones used in Setup
		...
		wheelEnt.scale = scale
		wheelEnt:SetWheelBase(trace.Entity)
		wheelEnt:SetModelScale(scale or 1,0)
		wheelEnt:GetPhysicsObject():SetMass(1)
...
function TOOL.BuildCPanel( panel )
	...
	panel:NumSlider("Size", "wire_wheel_scale", 0.1, 5, 5)
