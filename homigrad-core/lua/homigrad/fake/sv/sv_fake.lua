...
function PlayerMeta:CreateFake(force)
	...
	self.FakeActiveWeapon = self:GetActiveWeapon()
    ...
    if IsValid(activeWep) and activeWep:GetClass():StartWith("med_") then
        self.FakeMedicalWep = activeWep
    end
...
hook.Add("PlayerSwitchWeapon","Homigrad_Fake_Guns",function(ply,oldwep,wep)
    ...
    if ply.Fake and (wep:GetClass():StartWith("med_") or wep.SupportTPIK or wep:GetClass() == "weapon_base_grenade" or wep.Base == "weapon_base_grenade") then
        return false
    end
...
function Faking(ply,force)
	...
	if IsValid(ply.FakeRagdoll) and IsValid(ply.FakeRagdoll.MedicalWepModel) then
        ply.FakeRagdoll.MedicalWepModel:Remove()
    end
    ...
    if not ply.Fake then
		...
		if IsValid(ply.FakeMedicalWep) then
			ply:SelectWeapon(ply.FakeMedicalWep:GetClass())
			ply.FakeMedicalWep = nil
		elseif IsValid(ply.FakeActiveWeapon) then
			ply:SelectWeapon(ply.FakeActiveWeapon:GetClass())
			ply.FakeActiveWeapon = nil
		end
...
hook.Add("Player Collide","Ragdolling-Collide",function(ply,ent,data)
    ...
    local LIMIT_SPEED = 256
    
    if ROUND_NAME == "dr" then
        return
    end
    
    -- Добавляем проверку на Fake-режим
    if ply.Fake then return end
    
    -- Уточняем проверку физического объекта
    local phys = ent:GetPhysicsObject()
    if not IsValid(phys) then return end
    
    if (ent:GetVelocity():Length() > LIMIT_SPEED or phys:GetMass() > LIMIT_MASS and ent:GetVelocity():Length() > LIMIT_SPEED / 2) 
        and ent != ply and !ply:IsStandingOn(ent) then
        
        -- Добавляем задержку и дополнительные проверки
        timer.Simple(0,function()
