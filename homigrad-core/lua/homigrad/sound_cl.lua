...
function sound.Emit(ent,sndName,level,volume,pitch,poss)
    local pos = (IsValid(ent) and ent:GetPos() or poss)
    local _dsp = getDSP(pos)
    if not _dsp then return end

    EmitSound(sndName,pos,(ent and ent:EntIndex() or math.random(1,25252)),nil,volume,level,nil,pitch,_dsp == 0 and dsp or _dsp)
end
