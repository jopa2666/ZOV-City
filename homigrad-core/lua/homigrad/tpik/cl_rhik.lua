...
function hg.DoRHIK(wm,wep)
    ...
    if IsValid(rhik_model) then
        ...
        for _, bone in ipairs(hg.RHIKBones) do
    ...
    if IsValid(lhik_model) then
        ...
        for _, bone in ipairs(hg.LHIKBones) do
            local vm_bone = vm:LookupBone(bone)
...
function hg.RecalculateIKGunMotionOffset()
...
function hg.GunControllerRHIK(pos, ang)
    local seqproxy = self:GetSequenceProxy()
