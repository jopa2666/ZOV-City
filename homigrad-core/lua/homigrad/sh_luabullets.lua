...
local function Damage(bDoDebugHit, bStartedWater, bEndNotWater, iFlags, iDamage, iPlayerDamage, iNPCDamage, iAmmoDamage, pAttacker, pInflictor,
	...
	if (not bStartedWater and bEndNotWater or bit.band(iFlags, FIRE_BULLETS_DONT_HIT_UNDERWATER) == 0) then
		...
		if (fCallback) then
			fCallback(pAttacker, tr, info, tInfo)
		end
...
function ENTITY:FireLuaBullets(tInfo)
    ...
    local owner = tInfo.Attacker and tInfo.Attacker:IsValid() and tInfo.Attacker or self:GetOwner() and self:GetOwner():IsValid() and self:GetOwner() or self
    local bIsPlayer = owner:IsPlayer()

    if (bIsPlayer and not tInfo.DisableLagComp) then
    ...
    -- FireBullets info
    local iAmmoType = tInfo.AmmoType
    if iAmmoType == nil then
        iAmmoType = -1
    elseif isstring(iAmmoType) then
        iAmmoType = game.GetAmmoID(iAmmoType)
    end

    local pAttacker = tInfo.Attacker or self
    local fCallback = tInfo.Callback
    local iDamage = tInfo.Damage or 0
    local vDir = tInfo.Dir:GetNormal()
    ...
    do
        ...
        -- This is used to show where the lag compensator thinks the player should be at.
    ...
    for iShot = 1, iNum do
        ...
        -- If we're firing multiple shots, and the first shot has to be on target, ignore spread
        ...
        repeat
            ...
            local pEntity = tr.Entity
            bHitGlass = false -- Adjust this logic if needed
