SWEP.Base = "med_base"
SWEP.PrintName ="Salewa"
SWEP.Category = "Медицина"
SWEP.Spawnable = true
SWEP.SupportTPIK = true

SWEP.WorldModel = "models/carlsmei/escapefromtarkov/medical/salewa.mdl"
...
SWEP.WorldAng = Angle(0,165,90)
SWEP.WorldPos = Vector(0.9,-2,0)
SWEP.CorrectScale = 0.7
...
function SWEP:Heal(ply)
    ...
    if SERVER then
        ply.blood = math.Clamp(ply.blood + 85,0,5000)
        ply:SetHealth(math.Clamp(ply:Health() + math.random(8,18),0,ply:GetMaxHealth()))
        ply.bleed = math.Clamp(ply.bleed - math.random(17,24),0,1000)
        ply.painlosing = ply.painlosing + 0.6
    end
end
