...
function SWEP:IsAttacking(ply)
    if SERVER then
        local isAttacking = ply:KeyDown(IN_ATTACK) or (ply.Fake and ply:KeyDown(IN_ATTACK) and IsValid(ply:GetActiveWeapon()) and ply:GetActiveWeapon() == self)
        self:SetNWBool("in_attack", isAttacking)
        return isAttacking
    else
...
function SWEP:Step()
    ...
    if !IsValid(ply) then
        ...
        if ply.Fake and ply:GetActiveWeapon() != self then
        return
    end
