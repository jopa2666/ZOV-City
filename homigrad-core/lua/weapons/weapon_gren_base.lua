...
function SWEP:Step()
	...
	hg.bone.Set(ply,"r_forearm",Vector(0,0,0),Angle(15,-5,15),1,0.1)
	hg.bone.Set(ply,"r_upperarm",Vector(0,0,0),Angle(15,-60,0),1,0.1)
end
...
function SWEP:PrimaryAttack()
    ...
    timer.Simple(0.18,function()
        if SERVER then    
            TrownGranade(self:GetOwner(),800,self.Granade)
            self:Remove()
        elseif CLIENT then
        end
...
function SWEP:SecondaryAttack()
    ...
    self.Thrown = true
    if SERVER then
        TrownGranade(self:GetOwner(),275,self.Granade)
        self:Remove()
    elseif CLIENT then
    end
    self:EmitSound("weapons/m67/handling/m67_throw_01.wav")
