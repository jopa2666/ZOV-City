SWEP.Base = "med_base"
SWEP.PrintName = "Армейский бинт"
SWEP.Category = "Медицина"
SWEP.Spawnable = true
SWEP.SupportTPIK = true

SWEP.WorldModel = "models/carlsmei/escapefromtarkov/medical/bandage_army.mdl"
...
SWEP.WorldAng = Angle(0,-90,5)
SWEP.WorldPos = Vector(0.5,0,0)
SWEP.CorrectScale = 0.6
...
function SWEP:Heal(ply)
    ...
    if SERVER then
        if ply.bleed > 0 then
            ply.blood = math.Clamp(ply.blood + 35,0,5000)
        end
        ply.bleed = math.Clamp(ply.bleed - math.random(20,25),0,1000)
    end
end
