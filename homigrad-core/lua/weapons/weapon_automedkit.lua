SWEP.Base = "med_base"
SWEP.PrintName = "Автомобильная аптечка"
SWEP.Category = "Медицина"
SWEP.Spawnable = true
SWEP.SupportTPIK = true

SWEP.WorldModel = "models/carlsmei/escapefromtarkov/medical/automedkit.mdl"
...
function SWEP:Heal(ply)
    ...
    if SERVER then
        ply.blood = math.Clamp(ply.blood + 55,0,5000)
        ply:SetHealth(math.Clamp(ply:Health() + math.random(4,10),0,ply:GetMaxHealth()))
        ply.bleed = math.Clamp(ply.bleed - math.random(11,18),0,1000)
        ply.painlosing = ply.painlosing + 0.3
    end
end
