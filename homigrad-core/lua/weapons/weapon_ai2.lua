...
SWEP.WorldModel = "models/carlsmei/escapefromtarkov/medical/medkit.mdl"
...
SWEP.WorldPos = Vector(0.9,-1.5,0)
...
function SWEP:Heal(ply)
    ...
    if SERVER then
        ply.blood = math.Clamp(ply.blood + 30,0,5000)
        ply:SetHealth(math.Clamp(ply:Health() + math.random(4,7),0,ply:GetMaxHealth()))
        ply.bleed = math.Clamp(ply.bleed - math.random(9,11),0,1000)
        ply.painlosing = ply.painlosing + 0.2
    end
end
